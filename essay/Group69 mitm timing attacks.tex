...
To defend, the \textbf{MITRE D3FEND} framework lists countermeasures. Our D‑H defense matches ``Client‑server Mutual Authentication'' (D3‑CSMA) \cite{mitre_d3fend}. Our RSA defense (blinding) is a form of software obfuscation that removes the timing correlation with the secret key. Modern protocols such as TLS 1.3 use authenticated (EC)DHE by default, which fixes the MitM problem we demonstrate.
...
\begin{lstlisting}[caption={MitM Attack Logic in Python}, label={lst:mitm_code}]
# Doomfist intercepts the exchange
print("Doomfist intercepts and replaces keys...")
...
s_phara = get_shared_secret(m_public, 
    b_private)

# Doomfist computes secrets with both
s_doomfist_ana = get_shared_secret(a_public, 
    m_private)
s_doomfist_phara = get_shared_secret(b_public, 
    m_private)
\end{lstlisting}
...
From the textbook, a \textbf{Digital Signature} is a mathematical scheme for verifying the authenticity of digital messages or documents. It gives us:
\begin{itemize}
...
\begin{lstlisting}[caption={Vulnerable RSA Decryption}, label={lst:vuln_rsa}]
def vulnerable_decrypt(ciphertext, private_key):
    d, n = private_key
    result = 1
    d_bin = bin(d)[2:] # Binary string of d
    ...
    for bit in d_bin:
        ...
        if bit == '1':
            ...
            # This branch causes the timing leak
...
% --- FIXED: Manually broke long lines so they fit in IEEE column ---
\begin{lstlisting}[caption={Blinded RSA Decryption}, label={lst:blinded_rsa}]
def blinded_decrypt(ciphertext, pub_key, priv_key):
    e, n = pub_key
    d, n_priv = priv_key
    ...
    # 2. Blind: C' = C * r^e mod n
    c_prime = (ciphertext * pow(r, e, n)) % n
    
    # 3. Decrypt blinded: M' = (C')^d mod n
    # Even if this is vulnerable, input is random
    m_prime = vulnerable_decrypt(c_prime, 
        priv_key)
...
The results showed a clear and successful fix for the timing leak. The average time is slightly higher, which is expected due to the extra blinding operations. The key result is the huge drop in time variation (from $\sim$24.76 ms to $\sim$0.58 ms, a reduction of about 42$\times$). With blinding, the timing no longer matches the private key bits. The remaining variation is random noise that is not useful for an attacker.

The big drop in standard deviation confirms that the blinding factor effectively hides the private key's influence on execution time. Even if an attacker collects millions of samples, the random noise introduced by the blinding process $r$ washes out the tiny signal from the conditional multiplication.
...
\item \textbf{Jiachen Pan (20233802046):}
  Implemented Diffie–Hellman protocol, Man-in-the-Middle attack, and authenticated D–H defense (\texttt{code/dh\_exchange.py}). Wrote Methods (D–H) and Results (D–H) sections.

\item \textbf{Junlin Li (20233802058):}
  Implemented RSA helpers and key generation (\texttt{code/rsa\_utils.py}). Wrote Methods (RSA) and part of Related Work.

\item \textbf{Yueshen Wang (20233802018):}
  Implemented RSA timing attack and blinding defense (\texttt{code/rsa\_attacks.py}). Ran experiments and wrote Results (RSA) section.

\item \textbf{Junrui Liang (20233802068):}
  Wrote Abstract, Introduction, Discussion, and Conclusion. Converted the paper to IEEE LaTeX format and prepared Table 1.
\end{itemize}

...
\begin{enumerate}
    ...
    \end{itemize}
\end{enumerate}
Now they both have the secret number \textbf{2}, and no one else knows it. In the actual script, $a$ and $b$ are random per run, so concrete values will differ.
...
\paragraph{Parameter Binding and Message Format}
In production designs, the signed data must bind all security-relevant parameters and identities to avoid substitution and unknown key-share issues. Instead of signing only $A$, one should sign a structured message such as
\begin{equation}
\mathrm{Sig}_A\big(\,\mathrm{hash}(g \parallel p \parallel A \parallel \text{``Ana''} \parallel \text{``Phara''})\,\big),
\end{equation}
...
\begin{itemize}
    \item \textbf{Entry points:} Run \texttt{dh\_exchange.py} for D-H (normal, MitM, authenticated) and \texttt{rsa\_attacks.py} for RSA timing and blinding.
    \item \textbf{Non-determinism:} Both scripts use Python's \texttt{random} without a fixed seed. This is intentional for realism. For strict reproducibility, one can add \texttt{random.seed(\,)} with a fixed value at the top of each script.
    \item \textbf{Outliers:} We report average/minimum/maximum/variation directly from raw timings and do not remove outliers. Timing distributions in high-level languages may include occasional spikes from the OS scheduler or GC.
    \item \textbf{RSA blinding:} The blinding factor \(r\) must be invertible modulo \(n\). In practice this means ensuring \(\gcd(r, n) = 1\). Our code uses \texttt{pow(r, -1, n)}; if \(r\) is not invertible, one should resample \(r\) until it is. This edge case is rare but important in production systems.
    \item \textbf{Key sizes:} For timing visibility versus runtime, we chose a 512-bit RSA modulus in the experiment script. Real deployments require at least 2048-bit RSA.
\end{itemize}
...
\begin{itemize}
    \item D-H: both parties compute equal shared secret in the absence of an attacker.
    \item RSA: for random messages $M$ in $[1,n-1]$, \(\mathrm{dec}(\mathrm{enc}(M)) = M\).
    \item Signatures: \(\mathrm{verify}(A, \mathrm{Sig}_A(A), \mathrm{pk}_A) = \texttt{True}\) and forgeries fail.
\end{itemize}
For example, a minimal RSA check in Python:
\begin{lstlisting}[language=Python,caption={Sanity checks for RSA},label={lst:checks_rsa}]
...
\begin{itemize}
    ...
    \item \textbf{Constant-time caveat:} We did not implement a truly constant-time modular exponentiation. In high-level languages, hidden branches and optimizer behavior can reintroduce timing variance. Blinding is robust in practice, but constant-time code in a low-level language remains the gold standard.
    \item \textbf{Blinding factor invertibility:} The defense requires choosing \(r\) coprime with \(n\). Although the chance of collision with a prime modulus is negligible, production code must resample on failure and handle errors.
    \item \textbf{Noise sources:} CPU frequency scaling, OS scheduling, and background services can add variance unrelated to the key. Our experiment does not pin CPU cores or disable turbo/boost features.
\end{itemize}

...
if __name__ == "__main__":
    ...
    try:
        ...
        # Fallback if utils not available or different
        p = generate_prime_number(16)
        q = generate_prime_number(16)
    ...
    avg_vuln, var_vuln, times_vuln = run_timing_experiment(
        vulnerable_decrypt, 
        public_key, 
        private_key,
        num_trials=TRIALS
    ...
    avg_blind, var_blind, times_blind = run_timing_experiment(
        wrapper_blinded, 
        public_key, 
        private_key,
        num_trials=TRIALS
    ...
    try:
        ...
        print("\n[Optional] matplotlib found. Generating Histogram Plot...")
        plt.figure(figsize=(10, 6))
        plt.hist(times_vuln, bins=100, alpha=0.6, label='Vulnerable Decrypt', color='red')
        plt.hist(times_blind, bins=100, alpha=0.6, label='Blinded Decrypt (Secure)', color='green')
        plt.title('Execution Time Distribution: Vulnerable vs. Blinded RSA')
        plt.xlabel('Execution Time (nanoseconds)')
        plt.ylabel('Frequency')
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.savefig('rsa_timing_histogram.png')
        print("Chart saved as 'rsa_timing_histogram.png'.")
    except ImportError:
        print("\n[Note] matplotlib not found. Skipping plot generation (Data analysis is complete).")